...
export const AreaDetalle = ({ area, plantaName, onClose }) => {
    ...
    const total = area.totalTareas || 0;
    const activas = area.backlogActivo || 0;

    // BLINDAJE DE CACHÉ: Si el backend manda el formato viejo, lo adaptamos a la fuerza
    let tiposMap = area.tipos || {};
    if (Object.keys(tiposMap).length === 0) {
        tiposMap = {
            'TICKET': area.totalTickets || 0,
            'EXTRAORDINARIA': area.totalCorrectivosExtraordinarios || 0,
            'PLANEADA': 0
        };
    } else {
    ...
    const tiempoReal = area.tiemposCerradas?.tiempoRealTotal || 0;
    const tiempoEst = area.tiemposCerradas?.tiempoEstimadoTotal || 0;
    const cerradasTotales = area.tiemposCerradas?.cantidad || 0;
    ...
    return (
        <Modal isOpen onClose={onClose} className="md:max-w-3xl">
            <ModalHeader title={`Reporte Operativo: ${area.area}`} onClose={onClose} />
            <ModalBody className="max-h-[75vh] overflow-y-auto custom-scrollbar">
                <div className="flex flex-col gap-6 p-1">

                    <div className="flex items-center gap-2 bg-marca-primario/5 border border-marca-primario/20 px-4 py-3 rounded-xl">
                        <Icon name="domain" size="sm" className="text-marca-primario" />
                        <span className="text-sm font-bold text-marca-primario">Planta Perteneciente: {plantaName}</span>
                    </div>

                    {/* SECCIÓN 1: VOLUMEN DE CARGA */}
                    <div className="flex flex-col gap-3">
                        <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">1. Volumen de Carga</h4>
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="flex items-center justify-between">
                            <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">2. Tiempos Operativos</h4>
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <DistributionGrid data={area.estados} title="Estados Actuales" icon="timeline" />
                        <DistributionGrid data={area.clasificaciones} title="Clasificaciones" icon="category" />
                        <DistributionGrid data={area.categorias} title="Categorías Específicas" icon="label" />
                    </div>

                    {/* SECCIÓN 5: FRECUENCIA DE REPORTES */}
                    <div className="flex flex-col gap-3">
                        <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">4. Frecuencia Mensual de Reportes Cliente</h4>
                        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
                            {area.frecuenciaTickets && area.frecuenciaTickets.length > 0 ? (
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody>
                                        {area.frecuenciaTickets.map((f, idx) => (
