...
export default function HoyLayoutMobile() {
    ...
    const { moduleInfo, menuOptions } = useMemo(() => {
        const config = MODULES_CONFIG.find(m => m.id === 'hoy');
        const baseMenu = [
        ...
        const filtered = baseMenu.filter(item => {
            const childConfig = config?.children?.find(c => c.route === item.id);
            return childConfig ? childConfig.allowedRoles.includes(currentUser?.rol) : false;
        });

        return {
            moduleInfo: {
                title: config?.name || 'Hoy',
                description: 'Monitorea y ejecuta tus actividades preventivas y correctivas del día.'
            },
