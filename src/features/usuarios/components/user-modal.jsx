...
export const UserModal = ({ isOpen, onClose, onSuccess, usuarioAEditar, currentUser, departamentos }) => {
  ...
  const rolesDisponibles = [
    { value: "USUARIO", label: "Operativo" },
    { value: "ENCARGADO", label: "Supervisión" },
  ];
  ...
  const handleSubmit = async () => {
    if (!nombre.trim() || !username.trim() || !rol || (requiereDepartamento && !departamentoId))
    ...
    try {
      ...
      if (usuarioAEditar) {
        await updateUser(usuarioAEditar.id, payload);
      ...
      if (msg.toLowerCase().includes("unique") || msg.toLowerCase().includes("ya existe"))
        setBackendError(`Usuario "${username}" en uso.`);
      else
        toast.error(msg);
    } finally {
      ...
      <ModalBody>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select
                ...
                disabled={!requiereDepartamento || esAdmin}
              >
                {esAdmin ? (
          ...
          <div>
            <Label>
              Contraseña{" "}
              {usuarioAEditar && <span className="font-normal text-slate-400">(Dejar en blanco para conservar)</span>}
            </Label>
            <Input
              type="password"
              value={password}
